  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="query" size="small">
      <el-form-item label="模型名称">
        <el-input v-model.trim="query.name" placeholder="请输入名称" />
      </el-form-item>
      <el-form-item label="标识Key">
        <el-input v-model.trim="query.key" placeholder="请输入标识Key" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="list" stripe border style="width: 100%">
      ...
      <el-table-column align="center" prop="description" label="备注说明" min-width="130" />
      <el-table-column align="center" prop="createDate" label="创建时间" width="160" />
      <el-table-column align="center" prop="updateDate" label="更新时间" width="160" />
      <el-table-column align="center" label="操作" fixed="right" width="245">
        <template slot-scope="{row}">
          ...
          <el-popconfirm :title="`您确定部署【${row.name}】吗？`" style="margin: 0 5px" @onConfirm="clickDeploy(row.id)">
          ...
          <el-popconfirm :title="`您确定删除【${row.name}】吗？`" style="margin: 0 5px" @onConfirm="clickDelete(row.id)">
...
export default {
  ...
  methods: {
    ...
    async clickExportZip(id) {
      window.open(process.env.VUE_APP_BASE_URL + `/model/export/zip/${id}`)
    }
