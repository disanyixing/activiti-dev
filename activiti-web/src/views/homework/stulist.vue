<template>
  <div class="app-container">
    ...
    <el-table :data="papers" stripe border style="width: 100%">
      <el-table-column align="center" prop="title" label="答题名称" min-width="150" />
      ...
      <el-table-column align="center" label="状态" min-width="150">
        <template v-slot="{ row }">
          ...
          <el-tag v-if="row.graded && isAfterEnd(row.endDate)" type="success">
            已批改
          </el-tag>
        </template>
      </el-table-column>
  ...
  methods: {
    async fetchData() {
      ...
      this.papers = await Promise.all(response.data.records.map(async(paper) => {
        const studentsResponse = await course.getAllStudents(paper.courseId)
        ...
        paper.graded = Object.values(checkResults.studentResults).every(
          r => r.gradedAnswers === checkResults.totalQuestions
        )
